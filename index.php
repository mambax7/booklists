<?php
// $Id: index.php, v.0.8 2004/02/18 3:35 csloh  $
...
$xoopsOption['template_main'] = 'booklists_index.html';
include XOOPS_ROOT_PATH."/header.php";
$result=$xoopsDB->query("SELECT cid, title, imgurl FROM ".$xoopsDB->prefix("booklists_cat")." WHERE pid = 0 ORDER BY title") or exit("Error");
...
while($myrow = $xoopsDB->fetchArray($result)) {
	...
	foreach($arr as $ele){
		...
		$subcategories .= "<a href=\"".XOOPS_URL."/modules/booklists/viewcat.php?cid=".$ele['cid']."\">".$chtitle."</a>";
		$space++;
		$chcount++;
	}
...
list($numrows) = $xoopsDB->fetchRow($xoopsDB->query("select count(*) from ".$xoopsDB->prefix("booklists_links")." where status>0"));
...
$result = $xoopsDB->query("SELECT l.lid, l.cid, l.title, l.url, l.logourl, l.status, l.date, l.hits, l.rating, l.votes, l.comments, t.description FROM ".$xoopsDB->prefix("booklists_links")." l, ".$xoopsDB->prefix("booklists_text")." t where l.status>0 and l.lid=t.lid ORDER BY date DESC", $xoopsModuleConfig['newlinks'], 0);
while(list($lid, $cid, $ltitle, $url, $logourl, $status, $time, $hits, $rating, $votes, $comments, $description) = $xoopsDB->fetchRow($result)) {
	if ($isadmin) {
		$adminlink = '<a href="'.XOOPS_URL.'/modules/booklists/admin/?op=modLink&amp;lid='.$lid.'"><img src="'.XOOPS_URL.'/modules/booklists/images/editicon.gif" border="0" alt="'._MD_EDITTHISLINK.'" /></a>';
	} else {
		$adminlink = '';
	}
	if ($votes == 1) {
