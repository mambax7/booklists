...
if (!empty($HTTP_POST_VARS['submit'])) {
	...
	$rating = intval($HTTP_POST_VARS['rating']);
   	...
   	if ($ratinguser != 0) {
       	$result=$xoopsDB->query("select submitter from ".$xoopsDB->prefix("booklists_links")." where lid=$lid");
    	...
    	// Check if REG user is trying to vote twice.
   		$result=$xoopsDB->query("select ratinguser from ".$xoopsDB->prefix("booklists_votedata")." where lid=$lid");
       	while(list($ratinguserDB) = $xoopsDB->fetchRow($result)) {
       		if ($ratinguserDB == $ratinguser) {
		...
		$yesterday = (time()-(86400 * $anonwaitdays));
       	$result=$xoopsDB->query("select count(*) FROM ".$xoopsDB->prefix("booklists_votedata")." WHERE lid=$lid AND ratinguser=0 AND ratinghostname = '$ip' AND ratingtimestamp > $yesterday");
	...
	$newid = $xoopsDB->genId($xoopsDB->prefix("booklists_votedata")."_ratingid_seq");
	$datetime = time();
	$sql = sprintf("INSERT INTO %s (ratingid, lid, ratinguser, rating, ratinghostname, ratingtimestamp) VALUES (%u, %u, %u, %u, '%s', %u)", $xoopsDB->prefix("booklists_votedata"), $newid, $lid, $ratinguser, $rating, $ip, $datetime);
	...
	$xoopsOption['template_main'] = 'booklists_ratelink.html';
	include XOOPS_ROOT_PATH."/header.php";
	$lid = intval($HTTP_GET_VARS['lid']);
	$cid = intval($HTTP_GET_VARS['cid']);
	$result=$xoopsDB->query("select title from ".$xoopsDB->prefix("booklists_links")." where lid=$lid");
