...
function booklists_notify_iteminfo($category, $item_id)
{
	global $xoopsModule, $xoopsModuleConfig, $xoopsConfig;

	if (empty($xoopsModule) || $xoopsModule->getVar('dirname') != 'booklists') {	
		$module_handler =& xoops_gethandler('module');
		$module =& $module_handler->getByDirname('booklists');
	...
	if ($category=='category') {
		...
		$sql = 'SELECT title FROM ' . $xoopsDB->prefix('booklists_cat') . ' WHERE cid = '.$item_id;
	...
	if ($category=='link') {
		...
		$sql = 'SELECT cid,title FROM '.$xoopsDB->prefix('booklists_links') . ' WHERE lid = ' . $item_id;
		$result = $xoopsDB->query($sql); // TODO: error check
		$result_array = $xoopsDB->fetchArray($result);
		$item['name'] = $result_array['title'];
