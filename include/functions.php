...
function newlinkgraphic($time, $status) {
	...
	if ($startdate < $time) {
		if($status==1){
			$new = "&nbsp;<img src=\"".XOOPS_URL."/modules/booklists/images/newred.gif\" alt=\""._MD_NEWTHISWEEK."\" />";
		}elseif($status==2){
			$new = "&nbsp;<img src=\"".XOOPS_URL."/modules/booklists/images/update.gif\" alt=\""._MD_UPTHISWEEK."\" />";
		}
	}
	return $new;
}
...
function popgraphic($hits) {
	...
	if ($hits >= $xoopsModuleConfig['popular']) {
		return "&nbsp;<img src=\"".XOOPS_URL."/modules/booklists/images/pop.gif\" alt=\""._MD_POPULAR."\" />";
	}
	return '';
}
...
function updaterating($sel_id){
        global $xoopsDB;
        $query = "select rating FROM ".$xoopsDB->prefix("booklists_votedata")." WHERE lid = ".$sel_id."";
        //echo $query;
        ...
        $query =  "UPDATE ".$xoopsDB->prefix("booklists_links")." SET rating=$finalrating, votes=$votesDB WHERE lid = $sel_id";
        //echo $query;
...
function getTotalItems($sel_id, $status=""){
        ...
        $count = 0;
        $arr = array();
        $query = "select count(*) from ".$xoopsDB->prefix("booklists_links")." where cid=".$sel_id."";
        if($status!=""){
        ...
        $size = count($arr);
        for($i=0;$i<$size;$i++){
                $query2 = "select count(*) from ".$xoopsDB->prefix("booklists_links")." where cid=".$arr[$i]."";
                if($status!=""){
