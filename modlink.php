...
if (!empty($HTTP_POST_VARS['submit'])) {
	...
	$newid = $xoopsDB->genId($xoopsDB->prefix("booklists_mod")."_requestid_seq");
	$sql = sprintf("INSERT INTO %s (requestid, lid, cid, title, url, logourl, description, modifysubmitter) VALUES (%u, %u, %u, '%s', '%s', '%s', '%s', %u)", $xoopsDB->prefix("booklists_mod"), $newid, $lid, $cid, $title, $url, $logourl, $description, $user);
	...
	$xoopsOption['template_main'] = 'booklists_modlink.html';
	include XOOPS_ROOT_PATH."/header.php";
	$result = $xoopsDB->query("select cid, title, url, logourl from ".$xoopsDB->prefix("booklists_links")." where lid=$lid and status>0");
