...
$xoopsOption['template_main'] = 'booklists_viewcat.html';
...
if ( count($arr) > 0 ) {
    ...
    foreach($arr as $ele){
		...
		foreach($sub_arr as $sub_ele){
			...
			$infercategories .= "<a href=\"".XOOPS_URL."/modules/booklists/viewcat.php?cid=".$sub_ele['cid']."\">".$chtitle."</a>";
			$space++;
			$chcount++;
		}
...
$fullcountresult=$xoopsDB->query("select count(*) from ".$xoopsDB->prefix("booklists_links")." where cid=$cid and status>0");
...
if($numrows>0){
    ...
    $sql = "select l.lid, l.cid, l.title, l.url, l.logourl, l.status, l.date, l.hits, l.rating, l.votes, l.comments, t.description from ".$xoopsDB->prefix("booklists_links")." l, ".$xoopsDB->prefix("booklists_text")." t where cid=$cid and l.lid=t.lid and status>0 order by $orderby";
